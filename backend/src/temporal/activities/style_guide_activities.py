...
async def update_source_status_erreur_activity(source_id: str) -> None:
    logger.error(
        "Activity [A1-Rollback] : update_source_status_erreur_activity started", source_id=source_id
    )
    await asyncio.sleep(1)  # Simulation de l'appel DB
...
async def extract_rules_litellm_activity(chunk_ids: list[str]) -> str:
    logger.info(
        "Activity [A5] : extract_rules_litellm_activity started", input_chunks=len(chunk_ids)
    )
